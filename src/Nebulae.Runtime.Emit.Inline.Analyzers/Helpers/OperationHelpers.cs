...
namespace Nebulae.Runtime.Emit.Inline.Analyzers.Helpers
{
    ...
    {
        public static IMethodSymbol? GetEnclosingMethod(this IOperation operation, Compilation compilation)
        {
            SemanticModel model = compilation.GetSemanticModel(operation.Syntax.SyntaxTree);
            return model.GetEnclosingSymbol(operation.Syntax.SpanStart) as IMethodSymbol;
        }
            ...
            Func<IOperation, bool>? isEmpty = null)
        {
            ...
            {
                foreach (IOperation element in operation.ChildOperations)
                {
                    visitItem(element);
                }
