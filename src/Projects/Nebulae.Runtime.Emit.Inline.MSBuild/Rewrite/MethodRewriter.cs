...
{
    internal static class MethodRewriter
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void Elide(this Instruction instruction)
        {
            instruction.OpCode = OpCodes.Nop;
            instruction.Operand = Obsoletion;
        }
        ...
        public static void Rewrite(MethodDefinition definition)
        {
            try
            {
                var context = MethodRewriteContext.Create(definition);
                var placeholders = context.Placeholders;
                ...
                for (int i = 0; i < placeholders.Length; i++)
                {
                    ...
                    if (placeholder.IsPrimitive)
                    {
                        ...
                        instruction.Operand = placeholder.Operand.Resolve(instruction, context);
                        ...
                        else if (placeholder.Code is PlaceholderCode.Ret)
                        {
                            ...
                            case PlaceholderCode.Fail:
                                ...
                                if (instruction?.OpCode.Code is not Code.Throw)
                                {
                                    throw new InvalidProgramException(
                                        $"Cannot resolve the placeholder 'IL.Fail', " +
                                        $"the instruction sequence is incompatible.")
                                ...
                                instruction = instruction.Next;
                                break;
                            case PlaceholderCode.Pop:
                            case PlaceholderCode.Push:
                            case PlaceholderCode.Ret:
                                // The compiler-generated argument and result flow already
                                // has the required stack shape for Push, Pop, PopRef,
                                // PopPointer, and Ret. Remove only the placeholder call
                                // to preserve its value, managed reference, or pointer.
                                instruction.Elide();
                                break;
                            default:
        ...
        private static void Finalize(this MethodDefinition definition, in MethodRewriteContext context)
        {
        ...
        Retry:
            ...
            for (int i = 0; i < instructions.Count; i++)
            {
                ...
                if (opCode.OperandType is not OperandType.InlineBrTarget)
                {
                    continue;
                }

                // target - (start + op_size)
                int offset = ((Instruction)instruction.Operand).Offset - (instruction.Offset + opCode.Size);
        ...
        private static object? Resolve(this PlaceholderOperand type, Instruction instruction, scoped in MethodRewriteContext context)
        {
            return type switch
            {
        ...
        private static readonly object Obsoletion = new();
